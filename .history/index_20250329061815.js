...
const {ContactModel , ArticlesModel , UpcomingEventsModel , DueFeesDashboardModel , ClientsRatingModel , AvailableArticlesModel} = require('./contacts')
...
app.get('/availablearticles' , async (req , res) => {
    try{
      const clientsdata = await AvailableArticlesModel.find();
      console.log(clientsdata)
      res.json(clientsdata)
    } catch(err){
        console.error("Error fetching Due Fees details:" , err);
